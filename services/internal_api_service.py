...
def from_json_text(value: str | None) -> dict[str, Any]:
    if not value:
        return {}
    try:
...
def runtime_status_payload(status: RuntimeStatus | None) -> dict[str, Any]:
    ...
    payload = from_json_text(status.payload_json)
    return {
        "role": status.role,
...
def runtime_health_summary() -> dict[str, Any]:
    ...
    if metadata is None or metadata.status != "valid":
        ...
        next_action = "稍后重试；必要时重新扫码或更换网络/账号"
...
def pending_auth_runtime_state() -> dict[str, Any]:
    ...
    if worker.state == "bilibili_rejected":
        return {
            "status": "listener_unavailable",
            "http_status": 503,
            "next_action": "管理员可稍后重试；必要时重新扫码或更换网络/账号",
